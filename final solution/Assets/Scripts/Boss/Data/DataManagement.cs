...
public class DataManagement : MonoBehaviour
{
    ...
    private float takingDamagePunishment;
    ...
    private void Start()
    {
        ...
        hitPlayerReward = 8;//5
        blockPlayerReward = 20;//10
        goodBlockReward = 20;//7
        winReward = 10;
        missPlayerPunishment = -2;
        missBlockPunishment = -2;
        takingDamagePunishment = -3;
        losePunishment = -10;
        ...
        bossHealth = bController.GetCurrentHealth();
    }
    ...
    private void CalculateFitness(float hpBefore)
    {
        ...
        //devide by ten so that the gene pool is not huge when repopulation takes action
    ...
    private void Q_Training()
    {
        ...
        if (!bossDied && !playerDied)
        {
            if (!bAnimController.GetInAction() && !actionOnce)
            {
                ...
                //not taking damage as a knight or a warlock counts as a block
                if (bAnimController.GetDamageBlocked())
                {
                    stepReward = goodBlockReward;
                }

                if (bossHealth > bController.GetCurrentHealth() && !tookDamage)
                {
                    stepReward -= takingDamagePunishment;
                    tookDamage = true;
                }
            ...
            if(bAnimController.GetInAction() && actionOnce)
            {
                ...
                tookDamage = false;
                bAnimController.SetDamageBlocked(false);
